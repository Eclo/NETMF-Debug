...
namespace Microsoft.SPOT.Debugger
{
    public class Engine : IDisposable, IControllerHostLocal
    {
        ...
        private async Task<IncomingMessage> PerformRequestAsync(uint command, uint flags, object payload, int retries = 2, int timeout = 500)
        {
        ...
        public async Task<bool> WriteMemoryAsync(uint address, byte[] buf, int offset, int length)
        {
            ...
            while (count > 0)
            {
                ...
                IncomingMessage reply = await PerformRequestAsync(Commands.c_Monitor_WriteMemory, 0, cmd).ConfigureAwait(false);
