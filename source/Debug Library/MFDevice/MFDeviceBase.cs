    ...
    public class MFDeviceBase
    {
        ...
        private bool IsClrDebuggerEnabled
        {
            get
            {
                try
                {
                    if (DebugEngine.IsConnectedToTinyCLR)
                    {
        ...
        public async Task<bool> ConnectToTinyBooterAsync(CancellationToken cancellationToken)
        {
            bool ret = false;
            ...
            if (DebugEngine != null)
            {
                if (DebugEngine.ConnectionSource == ConnectionSource.TinyBooter) return true;
                ...
                for (int i = 0; i < 40; i++)
                {
                    ...
                    if (fConnected = await DebugEngine.ConnectAsync(0, 500, true, ConnectionSource.Unknown))
                    {
                        ...
                        ret = (reply.m_source == Commands.Monitor_Ping.c_Ping_Source_TinyBooter);
                ...
                if (!fConnected)
                {
                    Debug.WriteLine("Unable to connect to TinyBooter");
                }
            }
            return ret;
        }
        ...
        public async Task<bool> EraseAsync(EraseOptions options, CancellationToken cancellationToken, IProgress<ProgressReport> progress = null)
        {
            ...
            if (DebugEngine == null) throw new MFDeviceNoResponseException();
            ...
            {
                if (cancellationToken.IsCancellationRequested) throw new MFUserExitException();
            ...
            if (blocks.Count > 0)
            {
                ...
                foreach (SRecordFile.Block block in blocks)
                {
                    ...
                    if (DebugEngine.ConnectionSource != ConnectionSource.TinyCLR)
                    {
                        ...
                        if (!await DebugEngine.CheckSignatureAsync(((block.signature == null || block.signature.Length == 0) ? emptySig : block.signature), 0)) throw new MFSignatureFailureException(signatureFile);
                    }
                }
            }
        ...
        public async Task<bool> ExecuteAync(uint entryPoint, CancellationToken cancellationToken)
        {
            ...
            if (await CheckForMicroBooterAsync(cancellationToken))
            {
                ...
                if (m_execSrecHash.ContainsKey(entryPoint))
                {
                    ...
                    for (int retry = 0; retry < 10; retry++)
                    {
                        ...
                        await DebugEngine.SendBufferAsync(UTF8Encoding.UTF8.GetBytes(execRec), cancellationToken);
            ...
            if (reply.m_source == Commands.Monitor_Ping.c_Ping_Source_TinyBooter)
            {
                ...
                try
                {
                    ...
                    while (parsedFile.Records.Count > 0)
                    {
                        ...
                        foreach (uint key in keys)
                        {
                            ...
                            await DebugEngine.SendBufferAsync(UTF8Encoding.UTF8.GetBytes(parsedFile.Records[key]), cancellationToken);
                    ...
                    if (imageAddr != 0)
                    {
                        ...
                        if (folder.Path.ToLower().EndsWith("\\tinyclr.hex"))
                        {
                            binFilePath = Path.GetDirectoryName(path) + "\\tinyclr.bin\\" + basefile;
                            symdefFilePath = Path.GetDirectoryName(path) + "\\tinyclr.symdefs";
                        }
                        else
                        {
        ...
        private async Task PrepareForDeployAsync(List<SRecordFile.Block> blocks, CancellationToken cancellationToken, IProgress<ProgressReport> progress = null)
        {
            ...
            if (!IsClrDebuggerEnabled)
            {
                ...
                if (!await ConnectToTinyBooterAsync(cancellationToken)) throw new MFDeviceNoResponseException();
            }

           var flasSectorsMap = await DebugEngine.GetFlashSectorMapAsync();

            if (flasSectorsMap == null || flasSectorsMap.Count == 0) throw new MFDeviceNoResponseException();
            ...
            foreach (SRecordFile.Block bl in blocks)
            {
                foreach (Commands.Monitor_FlashSectorMap.FlashSectorData sector in flasSectorsMap)
                {
                    if (sector.m_address == bl.address)
                    {
                        ...
                        {
                            if (DebugEngine.ConnectionSource != ConnectionSource.TinyBooter)
                            {
                                progress?.Report(new ProgressReport(0, 1, "Connecting to TinyBooter..."));
            ...
            else if (DebugEngine.ConnectionSource != ConnectionSource.TinyBooter)
            {
                //if we are not writing to the deployment sector then assure that we are talking with TinyBooter
                await ConnectToTinyBooterAsync(cancellationToken);
            }
            if (DebugEngine.ConnectionSource == ConnectionSource.TinyCLR)
            {
                await DebugEngine.PauseExecutionAsync();
